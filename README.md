...
A Chrome MV3 extension that exposes your favorite search engines and live browser tabs as MCP tools. AI can operate web pages via JavaScript — click, fill forms, read SPA-rendered DOM, parse page JS state. Pure extension, no external dependencies, no setup. Load and go.

## Install

1. Download `mcp-browser-bridge-extension-v0.6.1.zip` (or the `.cxr` package) and unzip it.
...
The extension is reachable via Chrome's `externally_connectable` API from any web page:
...
lib/                   MCP/JSON-RPC parser, engine model, search runner, settings, i18n
